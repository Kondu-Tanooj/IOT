...
void loop() {
  ...
  if (acceleration > tapThreshold) {
    ...
    // If triple tap is detected, send SMS to designated numbers and make a phone call
    if (tapCount == 3) {
      Serial.println("3 Tapping");
      delay(5000);
      makePhoneCall(DESTINATION_NUMBER_1);
...
void makePhoneCall(const char* destinationNumber) {
  gsmSerial.print("ATD");
  gsmSerial.print(destinationNumber);
  gsmSerial.println(";");
  delay(5000); // Adjust the delay based on the desired call duration
